...
library(data.table)
library(ggsci)
load(file = "./0-Dataprepare/sce.all.Rdata")
load(file = "./0-Dataprepare/mycol.Rdata")
# sce.all = subset(sce.all, GSE == "GSE226602")
...
selected_c = WhichCells(sce.all, expression = (nFeature_RNA > 500) & (nFeature_RNA < 3000) & (nCount_RNA < 10000))
...
## 可视化过滤后的情况
feats = c("nFeature_RNA", "nCount_RNA")
VlnPlot(sce.all.filt, group.by = "orig.ident", features = feats, pt.size = 0, ncol = 1, cols = col_vec) + NoLegend()
ggsave(filename="./1-qc/1-Filter_Vlnplot1.pdf", width = 13, height = 8)

feats = c("percent_mito", "percent_hb")
VlnPlot(sce.all.filt, group.by = "orig.ident", features = feats, pt.size = 0, ncol = 1, cols = col_vec) + NoLegend()
ggsave(filename="./1-qc/2-Filter_Vlnplot2.pdf", width = 13, height = 8)
