...
# 细胞数目
meta = sce.all.filt.harmony@meta.data
meta = meta[!duplicated(meta$orig.ident),]
hcid = meta$orig.ident[meta$Group == "HC"]


cellcount = as.data.frame(prop.table(table(sce.all.filt.harmony$celltype, sce.all.filt.harmony$orig.ident), 2))
...
ggsave(p3all, filename = "./3-celltype/5-patient-proportion.pdf", width = 12, height = 2.5)



cellcount = as.data.frame(prop.table(table(sce.all.filt.harmony$celltype, sce.all.filt.harmony$Group), 2))
...
p1
ggsave(p1, filename = "./3-celltype/6-dot-group.pdf", width = 6, height = 4)
