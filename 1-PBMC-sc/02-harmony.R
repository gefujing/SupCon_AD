...
sce.all.filt = RunHarmony(sce.all.filt, group.by.vars = "GSE")
sce.all.filt = RunUMAP(sce.all.filt, dims = 1:20, reduction = "harmony")

DimPlot(object = sce.all.filt, group.by = "orig.ident", reduction = "umap", cols = col_vec)
ggsave(filename = "./2-harmony/1-dim-orig-spilt.pdf", width = 6.5, height = 4)
